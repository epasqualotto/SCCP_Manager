...
function make_speeddial_string($extension, $type){
	...
	// Enrico 05-12-2016
	// SpeedDial 8 prefix hardcoded
	if ($res != ''){
		$description = $res;
		
	} else {
		
		exec("/usr/sbin/asterisk -rx 'database showkey sysspeeddials/".substr($extension,1)."'",$output_1,$ret_1);
		$tmp_1 = explode(":",$output_1['0']);
		$num = trim($tmp_1['1']);
		exec("/usr/sbin/asterisk -rx 'database showkey cidname/".$num."'",$output_2,$ret_2);
		$tmp_2 = explode(":",$output_2['0']);
		$name = trim($tmp_2['1']);
...
function sccp_add_device($devData, $buttonData) {
    ...
    foreach ($devData as $Campo => $Valor){
		$ListaCampos .= ($ListaCampos ? "," : "")."$Campo=".(trim($Valor)!="" ? "\"$Valor\"" : "null");
		if (($Campo == "type")&&($Valor == "79XX+7914")){
			$ListaCampos .= ($ListaCampos ? "," : "")."addon=7914";
			$addon=1;
		} else if (($Campo == "type")&&($Valor == "79XX+7914+7914")){
			$ListaCampos .= ($ListaCampos ? "," : "")."addon=7914\;7914";
			//$ListaCampos .= ($ListaCampos ? "," : "")."addon=7914";
			$addon2=1;
		}			
	}
	...
	file_put_contents("/tftpboot/sep_debug.txt",$ListaCampos." \n=>\n ".print_r($buttonData,1),FILE_APPEND);
	...
	if (!$addon)
		...
		sccp_create_tftp_SEPaddon($devData['name']);
	else 	
		sccp_create_tftp_SEPaddon2($devData['name']);
}
...
function sccp_create_tftp_SEPaddon($device, $cm_ip = ''){
	if ($cm_ip == ''){
	...
	$template = file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/admin/modules/sccp_manager/SEPXML_7914.txt');
...
function sccp_create_tftp_SEPaddon2($device, $cm_ip = ''){
	if ($cm_ip == ''){
	...
	$template = file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/admin/modules/sccp_manager/SEPXML_7914_7914.txt');
